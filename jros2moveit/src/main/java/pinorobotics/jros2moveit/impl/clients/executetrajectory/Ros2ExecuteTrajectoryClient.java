...
public class Ros2ExecuteTrajectoryClient<
        ...
        extends AbstractExecuteTrajectoryClient<P, G, R> {
    ...
    @Override
    protected G createExecuteTrajectoryRequest(P plan) {
        var goalClass = actionDefinition.getActionGoalMessage().getMessageClass();
        if (goalClass.isAssignableFrom(
                pinorobotics.jros2moveit.moveit_msgs.humble.ExecuteTrajectoryActionGoalMessage
                        .class)) {
            var goal =
                    new pinorobotics.jros2moveit.moveit_msgs.humble.ExecuteTrajectoryGoalMessage();
            goal.trajectory =
                    ((pinorobotics.jros2moveit.moveit_msgs.humble.MoveGroupResultMessage) plan)
                            .planned_trajectory;
            return (G) goal;
        } else {
            var goal = new pinorobotics.jros2moveit.moveit_msgs.ExecuteTrajectoryGoalMessage();
            goal.trajectory =
                    ((pinorobotics.jros2moveit.moveit_msgs.MoveGroupResultMessage) plan)
                            .planned_trajectory;
            return (G) goal;
        }
    }
